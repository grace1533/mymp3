...
async function keepAlive() {
  ...
  try {
    ...
    if (error) {
      ...
      console.log(
        `[${new Date().toISOString()}] ✅ Storage API 检查成功，Buckets 数量: ${data?.length || 0}`
      );
  ...
  // ---------- 方法2: 数据库保活查询 ----------
  try {
    // 故意查询一个不存在的表，目的是产生数据库活动
    const { data, error } = await supabase
      .from(`_keep_alive_test_${Date.now()}`)
      .select("*")
      .limit(1);

    // 预期会报错（表不存在，错误码 42P01），但这足以保持连接活跃
    if (error && error.code === "42P01") {
      console.log(
        `[${new Date().toISOString()}] ✅ 保活查询执行成功（预期的表不存在错误）`
      );
  ...
  try {
    ...
    // 没有登录会话时返回 "Auth session missing!" 是正常的
